...
function HeroSlider() {
  ...
  return (
    ...
    >
      ...
      {/* Navigation arrows - Enhanced round design for mobile */}
      <button
        onClick={prevSlide}
      ...
      <div className="absolute bottom-16 sm:bottom-20 md:bottom-24 left-0 right-0 flex justify-center">
        ...
        >
          ...
          <div className="flex flex-col items-center">
            ...
            <svg
              className="w-5 h-5 sm:w-6 sm:h-6 animate-bounce group-hover:text-gold transition-colors duration-300"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2.5"
      ...
      <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 flex justify-center">
        <div className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-black/30 backdrop-blur-md rounded-full border border-white/20">
          {images.map((_, index) => (
            <button
              key={index}
